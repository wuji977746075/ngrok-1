...
install_go(){
	...
	if [ $(getconf WORD_BIT) = '32' ] && [ $(getconf LONG_BIT) = '64' ];then
		...
		if [ ! -f $SELFPATH/go1.7.6.linux-amd64.tar.gz ];then
			wget https://golang.google.cn/dl/go1.7.6.linux-amd64.tar.gz --no-check-certificate
		fi
	    tar zxvf go1.7.6.linux-amd64.tar.gz
	else
		if [ ! -f $SELFPATH/go1.7.6.linux-386.tar.gz ];then
			wget https://golang.google.cn/dl/go1.7.6.linux-386.tar.gz --no-check-certificate
		fi
...
case "$num" in
	...
	[8] )
		...
		echo "启动端口"
		read port
		nohup /usr/local/ngrok/bin/ngrokd -domain=$domain -httpAddr=":$port" > /home/rainbow/ngrok.out 2>&1 &
	;;
	[9] )
		echo "输入启动域名"
		read domain
